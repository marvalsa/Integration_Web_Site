  ...
  class CitiesSync {
    ...
    async insertCitiesIntoPostgres(cities) {
      ...
      try {
        console.log(
          `ℹ️ Iniciando procesamiento de ${uniqueCities.length} ciudades en PostgreSQL...`
        );
        ...
        for (const city of uniqueCities) {
          ...
          if (!cityId || !fullCityName) {
            console.warn(
              `⚠️ Registro de ciudad inválido: ${JSON.stringify(
                city
              )}. Omitiendo.`
            );
            errorCount++;
          ...
          if (!cityName) {
            console.warn(
              `⚠️ Nombre de ciudad vacío después de limpiar: "${fullCityName}". Omitiendo.`
            );
            errorCount++;
            continue;
          }
          ...
          try {
            ...
            if (
              dbError.code === "23505" &&
              dbError.constraint === "Cities_name_key"
            ) {
              console.error(
                `  ❌ Error de Unicidad para Ciudad ID ${cityId}: El nombre '${cityName}' ya está en uso por otra ciudad con un ID diferente. Omitiendo.`
              );
              errorCount++;
            } else {
              console.error(
                `  ❌ Error en BD al procesar Ciudad ID ${cityId} ('${cityName}'):`,
                dbError.message
              );
              errorCount++;
            }
        ...
        console.log(
          `✅ Procesamiento finalizado. ${processedCount} ciudades insertadas/actualizadas, ${errorCount} errores manejados.`
        );
      } catch (error) {
        console.error(
          `❌ Error crítico durante el procesamiento de ciudades en PostgreSQL:`,
          error
        );
        throw error;
      } finally {
    ...
    async run() {
      try {
        ...
        console.error(
          "🚨 ERROR CRÍTICO durante la sincronización de Ciudades. El proceso se detendrá."
        );
      } finally {
        if (this.pool) {
          console.log(
            "🔌 Cerrando pool de conexiones PostgreSQL para Ciudades..."
          );
          await this.pool.end();
