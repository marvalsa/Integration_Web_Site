...
class ZohoToPostgresSyncProjects {
    constructor() {
        ...
        try {
            ...
            console.log(`✅ Conectado al bucket de GCS: ${process.env.GCS_BUCKET_NAME}`);
        } catch (error) {
    ...
    async getZohoAccessToken() {
        try {
            ...
            console.error('❌ Error al obtener token de Zoho:', error.response?.data || error.message);
            throw error;
        }
    }
    ...
    async getZohoProjects(accessToken, offset = 0) {
        const coqlQueryObject = {
            select_query: `
                SELECT
                    ...
                    Habitaciones, Ba_os, Latitud, Longitud, Ciudad.id, Sala_de_ventas.id, Slug
        ...
        try {
            const response = await axios.post(`${this.zohoConfig.baseURL}/coql`, coqlQueryObject, {
                headers: {
    ...
    async getProjectAttributes(accessToken, parentId) {
        try {
            ...
            return response.data.data.map(attr => attr.Atributo?.id).filter(Boolean);
        } catch (error) {
    ...
    async getSalesRoomDetails(accessToken, salesRoomId) {
        ...
        try {
            ...
            console.error(`❌ Error al obtener detalles de la Sala de Ventas ID ${salesRoomId}:`, error.response?.data || error.message);
            return null;
        }
    }
    ...
    async insertProjectIntoPostgres(project, accessToken) {
        if (!project?.id) {
            console.warn('⚠️ Se intentó insertar un proyecto inválido o sin ID. Omitiendo.');
        ...
        const salesRoomId = project['Sala_de_ventas.id'];
        const salesRoomDetails = await this.getSalesRoomDetails(accessToken, salesRoomId);
        const attributeIdsArray = await this.getProjectAttributes(accessToken, hcValue);
        ...
        try {
            ...
            const statusFromZoho = project.Estado?.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim();
            const statusId = statusMap[statusFromZoho] || null;
            // AJUSTE: Convertir el ID de estado a formato JSONB de array
            const statusForDb = statusId ? JSON.stringify([statusId]) : null;

            // AJUSTE: Convertir el array de IDs de atributos a formato JSONB
            const attributesJson = attributeIdsArray?.length > 0 ? JSON.stringify(attributeIdsArray) : null;
            ...
            const values = [
                ...
                project['Mega_Proyecto.id'] ? project['Mega_Proyecto.id'].toString() : null, 
                statusForDb, // AJUSTE: Usar la versión JSON
        ...
        try {
            for (const t of typologies) {
                ...
                const values = [
                    t.id.toString(), 
                    projectHc.toString(), 
                    t.Nombre || null, t.Descripci_n || null,
    ...
    async getGCSFilePublicUrls(directoryPath) {
        try {
            ...
            return files
                ...
                .map(file => `https://storage.googleapis.com/${this.bucket.name}/${file.name}`);
        } catch (error) {
            console.error(`❌ Error al obtener URLs públicas de GCS para la ruta ${directoryPath}:`, error.message);
            return [];
        }
    }
    ...
    async syncProjectFilesInGCS(projectId, typologies) {
        ...
        try {
            ...
            console.error(`❌ Error al actualizar el proyecto ${projectIdStr} con las URLs de GCS:`, error);
        } finally {
            client.release();
        }
    }
    ...
    async run() {
        try {
            console.log('🚀 Iniciando sincronización de Proyectos, Tipologías y Archivos GCS...');
